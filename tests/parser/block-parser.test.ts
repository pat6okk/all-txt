...
describe('US-1.2: Block Parsing Logic', () => {
    ...
    it('should capture indented content until next task header', () => {
        const content = `TODO Main Task
    Some notes
    More notes
TODO Not part of task`;
        const tasks = parser.parseFile(content, 'test.md');

        expect(tasks).toHaveLength(2);
        expect(tasks[0].state).toBe('TODO');
        expect(tasks[0].blockContent).toEqual(['    Some notes', '    More notes']);
...
TODO Sibling`;
        ...
        expect(tasks[0].blockContent).toContain('    Child note');
        expect(tasks[1].text).toBe('Child');
        expect(tasks[1].indent).toBe('  ');
    });

    it('should terminate block on first non-empty dedented line', () => {
        const content = `TODO Main Task
    Notes
Text outside block
TODO Top Level Task`;
